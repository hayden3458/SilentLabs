...
const Team: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-nature-glass text-gray-900 font-inter flex flex-col relative overflow-hidden">
      ...
      <section className="flex flex-col items-center justify-center pt-16 pb-8 px-4">
        <h1 className="text-4xl md:text-5xl font-extrabold text-center mb-4 tracking-tight text-gray-900">Meet the Team</h1>
        <p className="text-lg md:text-xl text-gray-700 font-light text-center max-w-2xl mb-8">The people behind Promptly StartUp. We're a small group of builders, designers, and dreamers, driven by a passion for better AI interactions.</p>
        {/* Role Filter Chips */}
        <div className="flex flex-wrap gap-3 justify-center mb-6">
          {roles.map(role => (
            <button
              ...
              className={`px-4 py-1 rounded-full border transition-colors duration-200 font-medium text-sm ${selectedRole === role ? 'bg-glassgreen-500 text-glassblue-900 border-glassgreen-500' : 'bg-white/10 text-gray-80 border-white/20 hover:bg-glassgreen-400 hover:text-glassblue-900'}`}
            >
              {role}
            </button>
          ))}
      ...
      {isMobile ? (
        <section className="w-full max-w-xs mx-auto flex flex-col items-center mb-16 px-4">
          <div className="flex items-center justify-between w-full mb-4">
            ...
            <span className="text-sm text-gray-80">{carouselIdx + 1} / {filteredMembers.length}</span>
            ...
            >
              <div className={`transition-transform duration-700 transform-style-preserve-3d h-full w-full ${flipped === filteredMembers[carouselIdx].name ? 'rotate-y-180' : ''}`}>
                ...
                <div className="absolute inset-0 bg-white/20 backdrop-blur-glass rounded-2xl shadow-glass p-6 flex flex-col items-center text-center transition-transform duration-700" style={{ zIndex: 2, backfaceVisibility: 'hidden', background: 'linear-gradient(135deg, #4ADE80 0%, #38BDF8 100%)' }}>
                  ...
                  <p className="text-white text-sm mb-2">{filteredMembers[carouselIdx].bio}</p>
                  <div className="flex space-x-3 mb-2">
                    <a href={filteredMembers[carouselIdx].socials.linkedin} className="hover:text-glassgreen-500" aria-label="LinkedIn"><svg width="20" height="20" fill="white" className="inline"><path d="M16 8a6 6 0 0 1 6 6v6h-4v-6a2 2 0 0 0-4 0v6h-4v-6a6 6 0 0 1 6-6zM2 8h4v12H2V8zm2-4a2 2 0 1 1 0 4 2 2 0 0 1 0-4z"/></svg></a>
                    <a href={filteredMembers[carouselIdx].socials.github} className="hover:text-glassgreen-500" aria-label="GitHub"><svg width="20" height="20" fill="white" className="inline"><path d="M10 2C5 2 1 6 1 11c0 4 2.9 7.4 6.8 8.6.5.1.7-.2.7-.5v-1.7c-2.8.6-3.4-1.2-3.4-1.2-.4-1-1-1.3-1-1.3-.8-.6.1-.6.1-.6.9.1 1.4.9 1.4.9.8 1.4 2.1 1 2.6.8.1-.6.3-1 .5-1.2-2.2-.2-4.5-1.1-4.5-4.8 0-1 .4-1.8 1-2.5-.1-.2-.4-1.2.1-2.5 0 0 .8-.3 2.6 1a9 9 0 0 1 4.8 0c1.8-1.3 2.6-1 2.6-1 .5 1.3.2 2.3.1 2.5.6.7 1 1.5 1 2.5 0 3.7-2.3 4.6-4.5 4.8.3.3.6.8.6 1.7v2.5c0 .3.2.6.7.5C16.1 18.4 19 15 19 11c0-5-4-9-9-9z"/></svg></a>
                  </div>
                  <div className="text-xs text-white/90 italic">{filteredMembers[carouselIdx].fun}</div>
                </div>
                {/* Back Side (Flip) */}
                <div className="absolute inset-0 bg-white/30 backdrop-blur-glass rounded-2xl shadow-glass p-6 flex flex-col items-center text-center transition-transform duration-700 rotate-y-180" style={{ zIndex: 1, backfaceVisibility: 'hidden' }}>
                  ...
                  <div className="relative bg-gray-80 text-glassblue-900 rounded-xl px-4 py-2 mt-2 mb-2 animate-pulse shadow-lg">
                    <span className="block text-xs font-mono">{filteredMembers[carouselIdx].prompt}</span>
                    <span className="absolute left-4 -bottom-2 w-3 h-3 bg-gray-80 rotate-45" />
                  </div>
            ...
            >
              <div className={`transition-transform duration-700 transform-style-preserve-3d h-full w-full ${flipped === member.name ? 'rotate-y-180' : ''}`}>
                ...
                <div className="absolute inset-0 bg-white/20 backdrop-blur-glass rounded-2xl shadow-glass p-6 flex flex-col items-center text-center transition-transform duration-700" style={{ zIndex: 2, backfaceVisibility: 'hidden', background: 'linear-gradient(135deg, #4ADE80 0%, #38BDF8 100%)' }}>
                  ...
                  <div className="flex space-x-3 mb-2">
                    <a href={member.socials.linkedin} className="hover:text-glassgreen-500" aria-label="LinkedIn"><svg width="20" height="20" fill="white" className="inline"><path d="M16 8a6 6 0 0 1 6 6v6h-4v-6a2 2 0 0 0-4 0v6h-4v-6a6 6 0 0 1 6-6zM2 8h4v12H2V8zm2-4a2 2 0 1 1 0 4 2 2 0 0 1 0-4z"/></svg></a>
                    <a href={member.socials.github} className="hover:text-glassgreen-500" aria-label="GitHub"><svg width="20" height="20" fill="white" className="inline"><path d="M10 2C5 2 1 6 1 11c0 4 2.9 7.4 6.8 8.6.5.1.7-.2.7-.5v-1.7c-2.8.6-3.4-1.2-3.4-1.2-.4-1-1-1.3-1-1.3-.8-.6.1-.6.1-.6.9.1 1.4.9 1.4.9.8 1.4 2.1 1 2.6.8.1-.6.3-1 .5-1.2-2.2-.2-4.5-1.1-4.5-4.8 0-1 .4-1.8 1-2.5-.1-.2-.4-1.2.1-2.5 0 0 .8-.3 2.6 1a9 9 0 0 1 4.8 0c1.8-1.3 2.6-1 2.6-1 .5 1.3.2 2.3.1 2.5.6.7 1 1.5 1 2.5 0 3.7-2.3 4.6-4.5 4.8.3.3.6.8.6 1.7v2.5c0 .3.2.6.7.5C16.1 18.4 19 15 19 11c0-5-4-9-9-9z"/></svg></a>
                  </div>
                  <div className="text-xs text-white/90 italic">{member.fun}</div>
                </div>
                {/* Back Side (Flip) */}
                <div className="absolute inset-0 bg-white/30 backdrop-blur-glass rounded-2xl shadow-glass p-6 flex flex-col items-center text-center transition-transform duration-700 rotate-y-180" style={{ zIndex: 1, backfaceVisibility: 'hidden' }}>
      ...
      <section className="w-full max-w-6xl mx-auto mb-12 px-4">
        <h2 className="text-2xl font-bold text-gray-900 mb-4 text-center">Where We're From</h2>
        <p className="text-gray-80 text-center mb-6 max-w-2xl mx-auto">
        ...
        <div className="relative border-l-2 border-glassgreen-500 pl-8">
          {milestones.map((ms, idx) => (
            <div key={ms.date} className="mb-10 flex items-center group">
              <div className="absolute -left-4 w-8 h-8 flex items-center justify-center">
                <span className={`w-4 h-4 rounded-full bg-glassgreen-500 border-4 border-gray-900 shadow-lg transition-transform duration-300 group-hover:scale-125 animate-bounce`} />
              </div>
              <div>
                <div className="text-sm text-gray-80 font-semibold">{ms.date}</div>
                <div className="text-lg text-gray-90 font-bold">{ms.label}</div>
              </div>
            </div>
          ))}
          ...
          {teamMembers.map((member, idx) => (
            <div key={member.name + '-join'} className="mb-10 flex items-center group">
              <div className="absolute -left-4 w-8 h-8 flex items-center justify-center">
                <span className="w-4 h-4 rounded-full bg-glassblue-700 border-4 border-gray-900 shadow-lg transition-transform duration-300 group-hover:scale-125 animate-pulse" />
              </div>
              <div>
                <div className="text-sm text-gray-80 font-semibold">{member.joinDate}</div>
      ...
      <section className="w-full max-w-4xl mx-auto flex flex-col md:flex-row gap-4 justify-center items-center mb-16 px-4">
        {values.map((val) => (
          <div key={val.title} className="flex-1 bg-white/20 backdrop-blur-glass rounded-2xl shadow-glass p-6 flex flex-col items-center text-center min-w-[180px] hover:scale-105 transition-transform duration-300">
            ...
            <p className="text-gray-90 text-sm">{val.desc}</p>
          </div>
        ))}
      </section>
      ...
      <section className="flex flex-col items-center mb-12">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Want to join our story?</h2>
